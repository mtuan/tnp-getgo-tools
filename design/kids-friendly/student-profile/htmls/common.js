...
document.querySelector('#app').innerHTML = `
  <header class="topbar"><div class="brand"><span class="brand-mark" aria-hidden="true">G</span><span>Hồ sơ</span><span class="desktop-brand">GetGo Local</span></div><nav class="desktop-nav" aria-label="Điều hướng học sinh"><a href="#learning">Học tập</a><a href="#activities">Hoạt động</a><a href="#ranking">Bảng xếp hạng</a><a href="#profile" aria-current="page">Hồ sơ</a></nav><div class="top-actions"><span class="stars" aria-label="0 ngôi sao">★ 0</span><div class="account"><img src="../assets/student-charly-avatar.png" alt=""><span class="account-copy"><strong>Mai Thanh Ngọc (Charly)</strong><small>★ 0 ngôi sao</small></span><span aria-hidden="true">⌄</span></div></div></header>
  <section class="profile-banner" aria-hidden="true"><div class="theme-controls"><button class="theme-control" type="button" aria-label="Ngôn ngữ">◎</button><button class="theme-control" type="button" aria-label="Màu chủ đạo">◉</button><button class="theme-control js-theme-toggle" type="button" aria-pressed="false" aria-label="Chuyển sang chế độ tối">◐</button></div></section>
  <div class="content-rail"><section class="identity" aria-labelledby="student-name"><div class="avatar"><img src="../assets/student-charly-avatar.png" alt="Ảnh đại diện của Mai Thanh Ngọc"></div><div class="identity-copy"><h1 id="student-name">Mai Thanh Ngọc (Charly)</h1><p>Lớp 3 · Nữ</p></div><button class="edit-profile" type="button" aria-label="Chỉnh sửa hồ sơ">${icons.edit}</button></section>
  ...
  <div class="footer-art" aria-hidden="true"></div><div class="bottom-safe-space" aria-hidden="true"></div>
  <nav class="bottom-nav" aria-label="Điều hướng học sinh"><a href="#learning">${icons.book}<span>Học tập</span></a><a href="#activities">${icons.activity}<span>Hoạt động</span></a><a href="#ranking">${icons.trophy}<span>Bảng xếp hạng</span></a><a href="#profile" aria-current="page">${icons.user}<span>Hồ sơ</span></a></nav>`;
const themeToggle = document.querySelector('.js-theme-toggle');
const themeRow = document.querySelector('.js-theme-row');
const themeValue = document.querySelector('.js-theme-value');
const setTheme = dark => { root.dataset.theme = dark ? 'dark' : 'light'; themeToggle.setAttribute('aria-pressed', String(dark)); themeToggle.setAttribute('aria-label', dark ? 'Chuyển sang chế độ sáng' : 'Chuyển sang chế độ tối'); themeValue.textContent = dark ? 'Tối' : 'Sáng'; };
if (root.dataset.mode === 'responsive') { setTheme(matchMedia('(prefers-color-scheme: dark)').matches); themeToggle.addEventListener('click', () => setTheme(root.dataset.theme !== 'dark')); themeRow.addEventListener('click', () => setTheme(root.dataset.theme !== 'dark')); } else { setTheme(fixedTheme === 'dark'); themeToggle.disabled = true; themeRow.disabled = true; }
