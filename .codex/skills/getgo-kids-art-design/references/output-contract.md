...
Record schema version, page slug/name/route, sources, canonical viewport sizes, art direction, tokens, overlay color/opacity, safe zones, shared semantic content, orientation composition rules, page height behavior (`fixed` or `scrolling`), and proof that dark mode shares assets and geometry.

For every asset record stable ID, relative file, role, dimensions, orientation support, anchor, focal point, crop behavior, and `backgroundMode` (`transparent`, `removal-matte`, or `opaque-background`). Scrolling pages must identify separate `header` and `footer` edge assets, their full-width behavior, and `document-top` / `document-bottom` anchors. Record `removalMatte`, normally `#8B00FF`, when applicable.
...
Record pass/fail and evidence for required files, dimensions, source-content and semantic-control parity, local-only HTML, orientation recomposition, light/dark asset and geometry parity, overlay behavior, theme-neutral backgrounds, natural object placement, cross-asset style/identity consistency, built-in-only generation provenance, matte/alpha inspection, keyboard/focus, resize, short viewport, tall scrolling content, 200% zoom, and four rendered comparisons. Mark unavailable checks as unavailable, never passed.
