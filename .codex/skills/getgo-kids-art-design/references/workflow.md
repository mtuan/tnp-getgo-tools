...
Create one shared component/layout model. Every HTML file synchronously loads `../../shared/demo-shell.css` before page CSS and declares its shell class in markup: `page kids-bounded-page` for `vertical-scroll`, or `page kids-fullscreen-page` for `fullscreen-fixed`. Never add this class through JavaScript. The shared shell owns the 1024px boundary, canvas, canonical `rgba(0, 32, 27, .72)` dark artwork mask, edge-art containment, and fixed bar alignment; page CSS must not restate or override those rules. Four fixed files lock viewport and theme for comparison. `responsive.html` uses CSS media/container queries to recompose between portrait and landscape and includes a visible, keyboard-operable light/dark toggle. Dark state applies the shared overlay and tokens without changing artwork files or element geometry within an orientation.
