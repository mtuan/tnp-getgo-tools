...
For scrolling pages, create orientation-specific full-width header and footer scenery as separate transparent assets. Generate portrait edge assets at exactly `1170px` intrinsic width for 3× display at `390px`, and landscape edge assets at exactly `2048px` intrinsic width for 2× display at up to `1024px`; choose and record the proportional height required by the composition. Size and crop each for its edge role; fade or taper the inward-facing illustrated silhouette into real alpha so the code-native page color remains visible behind it. Do not generate an opaque sky or page-color field as part of an edge asset. Use an orientation-specific full background only when the page is demonstrably fixed-height and cannot scroll; generate it at exactly `1170 × 2532px` for portrait or `2048 × 1536px` for landscape. Create each foreground character/object as its own cut asset. Prefer real alpha; otherwise use the solid purple removal-matte contract in `SKILL.md`. Never ship a generated checkerboard as transparency.
