...
For scrolling pages, create orientation-specific full-width header and footer scenery as separate transparent assets. Generate portrait edge assets at exactly `1170px` intrinsic width for 3× display at `390px`, and landscape edge assets at exactly `2048px` intrinsic width for 2× display at up to `1024px`; choose and record the proportional height required by the composition. Size and crop each for its edge role; fade or taper the inward-facing illustrated silhouette into real alpha so the code-native page color remains visible behind it. Do not generate an opaque sky or page-color field as part of an edge asset. Use an orientation-specific full background only when the page is demonstrably fixed-height and cannot scroll; generate it at exactly `1170 × 2532px` for portrait or `2048 × 1536px` for landscape.

Create each foreground character/object as its own cut asset. Before generation, inventory its maximum CSS display box across both layouts. If pose/composition/crop is shared, generate one transparent source at least 3× that maximum box and reuse it in both orientations; landscape scales the same source down. Generate separate orientation variants only when the visual composition or maximum logical box changes, and record the reason. Apply the same style-lock/family ID, identity reference, padding convention, and alpha/matte process to every member of the asset set. Prefer real alpha; otherwise use the solid purple removal-matte contract in `SKILL.md`. Never ship a generated checkerboard as transparency.
...
Create one responsive component/layout model. `index.html` synchronously loads `../shared/common.css` before `index.css` and `../shared/common.js` before `index.js`, and declares `page kids-bounded-page` for `vertical-scroll` or `page kids-fullscreen-page` for `fullscreen-fixed`. Never add this class through JavaScript. Shared code owns the 1024px boundary, canvas, canonical `rgba(0, 32, 27, .72)` dark artwork mask, edge-art containment, fixed bar alignment, theme controller, and reusable navigation/background/decorative markup. The page entrypoint uses CSS media/container queries to recompose between portrait and landscape and includes a visible, keyboard-operable light/dark toggle. Dark state applies the shared overlay and tokens without changing artwork files or element geometry within an orientation.

Reconstruct the captured UI first and compare its geometry before adding any artwork. Paint one solid CSS page background behind the entire document, then layer the transparent header and footer PNGs above that same color. Do not assign separate matching colors to hide opaque image rectangles. Add backgrounds and decorations as non-participating layers (`position: absolute` or equivalent) so they cannot affect intrinsic size, flex/grid allocation, wrapping, or document flow. Edge-art wrappers must use explicit portrait/landscape height caps and `overflow: hidden`; scale images to full page width with preserved aspect ratio and anchor each crop toward the painted outer edge identified through alpha inspection (normally top for a top-painted header and bottom for a bottom-painted footer). Never create content padding, margins, or spacers for decorative artwork. Preserve existing UI surfaces and tokens unless a minimal contrast adjustment is required for readability over the new artwork; such an adjustment must not change geometry or component presentation.

Do not use canvas, remote fonts/libraries, embedded base64, or a full-page demo PNG as reconstruction content.

Place fixed application headers at exactly `top: 0` and fixed bottom navigation/footer surfaces at exactly `bottom: 0`. Keep these bar surfaces above the artwork mask and sufficiently opaque that transparent edge artwork cannot leak through them. If the design-gallery device frame needs anti-gap handling, overscan and clip the iframe outside the canonical visible `390 × 844` page; never compensate inside a generated page.

## 6. Complete source and package validation

Do not discover, connect to, or launch a browser, Electron application, headless runtime, or screenshot renderer. Do not search installed applications, caches, dependencies, or processes for one. Validate the responsive entrypoint, page and shared dependencies, assets, manifests, source content, layout-lock rules, theme parity, and responsive CSS through source and package inspection.
