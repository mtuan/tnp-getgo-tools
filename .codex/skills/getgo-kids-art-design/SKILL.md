...
- For every button variant in the page—including neutral, provider/social, primary, secondary, destructive, disabled, hover, active, and focused states—define theme-aware background, text/icon, and border colors with readable contrast. Muted text and legal/help links also require explicit dark tokens; a dark panel with unchanged light-mode button/text styling is a rejection.
