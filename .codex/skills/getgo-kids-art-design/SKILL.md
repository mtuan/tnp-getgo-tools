...
- Anchor a clipped header image to the wrapper's bottom edge so the image's bottom portion remains visible; anchor a clipped footer image to the wrapper's top edge so the image's top portion remains visible. Keep the wrapper overflow hidden, document the cap for portrait and landscape, and verify that the crop does not expose a hard image boundary or obscure content.
