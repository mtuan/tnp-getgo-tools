...
import { canonicalQuestionJson, hashPublishedQuestions, sanitizePublishedQuestion } from "../src/core/publishing.js"
import { recordPublishedHash } from "../src/repositories/quiz-publishing.js"

const dynamic = {
  paramsGeneratorTs: "() => ({ value: QB.rnd.int(1, 9) })",
  questionGeneratorTs: "({ value }) => ({ question_no: 1, text_en: `${value}`, answer: { type: 'input', correct: `${value}` } })",
...
test("publishes only allowlisted runtime question fields and dynamic fragments", () => {
  ...
  assert.deepEqual(Object.keys(question).sort(), ["answer", "category", "dynamic", "explanation", "question_no", "text_en", "text_vn"])
  assert.deepEqual(Object.keys(question.dynamic!).sort(), ["explanationGeneratorTs", "originParamsTs", "paramsGeneratorTs", "questionGeneratorTs"])
...
test("AI and review changes do not affect hashes, but dynamic code changes do", () => {
  const base = { question_no: 1, text_en: "Value?", answer: { type: "input", correct: "4" }, advancedDynamic: dynamic }
  const first = sanitizePublishedQuestion({ ...base, verified: false, aiResponse: { generatedAt: "one" } as never })
  const second = sanitizePublishedQuestion({ ...base, verified: true, aiResponse: { generatedAt: "two" } as never })
  assert.equal(hashPublishedQuestions([first]), hashPublishedQuestions([second]))
  const changed = sanitizePublishedQuestion({ ...base, advancedDynamic: { ...dynamic, paramsGeneratorTs: "() => ({ value: 5 })" } })
...
test("canonical hashing ignores question and object-key ordering", () => {
  ...
  assert.equal(hashPublishedQuestions([one, two]), hashPublishedQuestions([two, one]))
  assert.equal(canonicalQuestionJson([two, one]), canonicalQuestionJson([one, two]))
})

test("image contents and non-asset references cannot be published", () => {
  assert.throws(() => sanitizePublishedQuestion({ question_no: 1, text_en: "Image", image_datas: ["data:image/png;base64,abc"], answer: { type: "input", correct: "1" } }), /asset references/)
})

test("records the last successful published hash in the local quiz manifest", async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "getgo-publishing-"))
  t.after(() => fs.rm(directory, { recursive: true, force: true }))
  const manifestPath = path.join(directory, "manifest.json")
  await fs.writeFile(manifestPath, JSON.stringify({ id: "quiz", untouched: true }))
  ...
  assert.deepEqual(saved, { id: "quiz", untouched: true, publishedHash: hash, publishedAt: "2026-08-04T01:00:00.000Z" })
})
