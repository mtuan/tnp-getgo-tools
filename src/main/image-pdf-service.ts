...
import type { ImagePdfInput, ImagePdfOrientation, ImagePdfSelection } from "../core/models.js";
...
export async function loadImagePdfSelection(inputPaths: string[]): Promise<ImagePdfSelection> {
  ...
  const images: ImagePdfInput[] = await Promise.all([...new Set(files)].map(async (filePath) => {
    ...
    return { path: filePath, directory: path.dirname(filePath), name: path.basename(filePath), size: stats.size, mimeType: mimeTypes[path.extname(filePath).toLowerCase()], data: bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) as ArrayBuffer };
  }));
...
async function detectImageOrientation(filePath: string): Promise<ImagePdfOrientation> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(executable, [filePath, "stdout", "--psm", "0"], { stdio: ["ignore", "pipe", "pipe"] });
    let output = "";
    child.stdout.on("data", (chunk) => { output += String(chunk); });
    ...
    child.on("close", async () => {
      ...
      const deskewRotation = detected && raw === 0 && source === 0 ? await deskew(executable, filePath) : 0;
