...
export function parseFourLetterIcon(value?: unknown): FourLetterIconValue | null {
  ...
  if (!icon) return null
  const theme = textContentIconThemes.find(candidate => textContentIconColors[candidate] === icon.backgroundColor) ?? "violet"
  return { code: icon.text, backgroundColor: icon.backgroundColor, textColor: icon.textColor, theme }
}

export function encodeFourLetterIcon(code: string, theme: FourLetterIconTheme, textColor = "#ffffff") {
  return createTextContentIcon(code, textContentIconColors[theme], textColor)
}

export function FourLetterIcon({ code, backgroundColor, textColor = "#ffffff", theme = "violet", label, className = "" }: { code: string; backgroundColor?: string; textColor?: string; theme?: FourLetterIconTheme; label?: string; className?: string }) {
  ...
  const resolvedColor = backgroundColor ?? textContentIconColors[theme]
  return <span className={`four-letter-icon ${className}`.trim()} style={{ "--monogram-color": resolvedColor, "--monogram-text": textColor } as CSSProperties} role="img" aria-label={label ? `${label} (${letters.join("")})` : letters.join("")}>
