...
export function TreeDataTable<T>({ rows, columns, rowKey, ariaLabel, emptyText = "No rows yet.", onRowClick, toggleParentOnRowClick = false, singleExpand = false, onExpand, renderIdentity }: {
  rows: TreeDataRow<T>[];
  ...
  const renderRows = (items: TreeDataRow<T>[], depth: number): ReactNode => items.flatMap((item, index) => {
    ...
    const row = <tr key={key} className={[(onRowClick || (toggleParentOnRowClick && branch)) && "clickable", depth === 0 ? "ui-tree-data-parent" : "ui-tree-data-child"].filter(Boolean).join(" ")} onClick={toggleParentOnRowClick && branch ? () => void toggleRow(item, key) : onRowClick ? () => onRowClick(item.row) : undefined}><DataTableCells columns={effectiveColumns} row={item.row} index={index} /></tr>;
  ...
  return <div className="ui-data-table ui-tree-data-table"><div className="ui-data-table-scroll"><table aria-label={ariaLabel}><DataTableColumns columns={columns} /><thead><DataTableHeader columns={columns} /></thead><tbody>{renderRows(rows, 0)}{!rows.length && <tr><td className="ui-data-table-empty" colSpan={columns.length}>{emptyText}</td></tr>}</tbody></table></div></div>;
}
