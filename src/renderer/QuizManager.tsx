...
import { ArrowDown, ArrowUp, Check, ChevronRight, Eye, FolderOpen, ListOrdered, Plus, RefreshCw, RotateCcw, Save, Search, Trash2, Zap } from "lucide-react"
...
export function QuizManager({ snapshot, initialRoute, onSnapshotChange, onRouteChange, onBackActionChange }: QuizManagerProps) {
  ...
  if (page.kind === "quiz") {
    ...
    const questionColumns: DataColumn<QuestionListItem>[] = [
      ...
      ...(questionOrder ? [{ key: "order", title: "Order", width: 92, render: (item: QuestionListItem) => { const index = questionOrder.indexOf(item.number); return <span className="question-order-actions"><Button variant="icon" icon={<ArrowUp />} disabled={index <= 0} aria-label={`Move question ${item.number} up`} title="Move up" onClick={event => { event.stopPropagation(); moveOrderedQuestion(item.number, -1) }} /><Button variant="icon" icon={<ArrowDown />} disabled={index < 0 || index >= questionOrder.length - 1} aria-label={`Move question ${item.number} down`} title="Move down" onClick={event => { event.stopPropagation(); moveOrderedQuestion(item.number, 1) }} /></span> } }] : []),
      ...(!questionOrder ? [{ key: "preview", title: "", width: 56, render: (item: QuestionListItem) => <TableActionButton icon={<Eye size={16} strokeWidth={2.25} />} color="primary" aria-label={`Preview question ${item.number}`} title="Preview question" onClick={event => { event.stopPropagation(); setPreviewQuestion(item.record) }} /> }] : []),
    ]
