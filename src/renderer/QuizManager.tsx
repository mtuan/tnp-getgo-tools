...
import { ArrowDown, ArrowUp, Check, ChevronRight, FolderOpen, ListOrdered, Pencil, Plus, RefreshCw, RotateCcw, Save, Search, Trash2, Zap } from "lucide-react"
...
type QuizDetailTab = "questions" | "info"
...
function quizReviewStatus(quiz: QuizSummary): { kind: "full" | "partial" | "none"; label: string; reviewed: number; total: number } {
  ...
  if (total > 0 && reviewed === total) return { kind: "full", label: "Fully reviewed", reviewed, total }
  if (reviewed > 0) return { kind: "partial", label: "Partially reviewed", reviewed, total }
  return { kind: "none", label: "Not reviewed", reviewed, total }
}

function restoredPage(snapshot: RepositorySnapshot, route?: string): { page: ManagerPage; questionNo: string | null; questionTab: QuestionEditorTab; quizTab: QuizDetailTab } {
  if (!route?.startsWith("/quizzes/contests/")) return { page: { kind: "contests" }, questionNo: null, questionTab: "static", quizTab: "questions" }
  let url: URL
  ...
  const quizTab: QuizDetailTab = !isQuestionRoute && url.searchParams.get("tab") === "info" ? "info" : "questions"
  const contest = snapshot.contests.find(item => item.id === parts[2])
  if (!contest) return { page: { kind: "contests" }, questionNo: null, questionTab, quizTab }
  if (parts[3] !== "quizzes" || !parts[4]) return { page: { kind: "contest", contest: contest.id }, questionNo: null, questionTab, quizTab }
  const quiz = snapshot.quizzes.find(item => item.contest === contest.id && item.id === parts[4])
  if (!quiz) return { page: { kind: "contest", contest: contest.id }, questionNo: null, questionTab, quizTab }
  return { page: { kind: "quiz", quiz }, questionNo: isQuestionRoute ? parts[6] : null, questionTab, quizTab }
}
...
export function QuizManager({ snapshot, initialRoute, onSnapshotChange, onRouteChange, onBackActionChange }: QuizManagerProps) {
  ...
  const [previewQuestion, setPreviewQuestion] = useState<QuizQuestionRecord | null>(null)
  ...
  useEffect(() => {
    ...
    if (page.kind === "quiz") onRouteChange(`/quizzes/contests/${encodeURIComponent(page.quiz.contest)}/quizzes/${encodeURIComponent(page.quiz.id)}${pendingQuestionNo ? `/questions/${encodeURIComponent(pendingQuestionNo)}?tab=${questionEditorTab}` : `?tab=${quizTab}`}`)
  }, [onRouteChange, page, pendingQuestionNo, questionEditorTab, quizTab])
  ...
  if (page.kind === "quiz") {
    ...
    const displayedQuestions = questionOrder
      ? questionOrder.map(number => questions.find(question => question.number === number)).filter((question): question is QuestionListItem => Boolean(question))
      : questions
    ...
    const moveOrderedQuestionTo = (fromIndex: number, toIndex: number) => {
      setQuestionOrder(current => {
        if (!current || fromIndex < 0 || toIndex < 0 || fromIndex >= current.length || toIndex >= current.length) return current
        const next = [...current]
    ...
    return <section className="manager editor-page">
      <PageHeader eyebrow="Quiz detail" breadcrumbs={[{ label: "Contests", onClick: () => setPage({ kind: "contests" }) }, { label: quizContest?.title ?? quiz.contest.toUpperCase(), onClick: goBack }]} title={quiz.title} description={`${quiz.id} · ${[quiz.grade && `Grade ${quiz.grade}`, quiz.round, quiz.year].filter(Boolean).join(" · ")}`} titleAction={<Button className="ui-page-header-folder" icon={<FolderOpen />} variant="icon" disabled={Boolean(buttonAction)} aria-label="Show quiz in folder" title="Show quiz in folder" onClick={() => void runButtonAction("show-quiz-folder", () => window.getgo.showInFolder(quiz.manifestPath))} />} actions={quizTab === "info" ? <Button icon={<Trash2 size={15} />} loading={buttonAction === "delete-quiz"} variant="solid" color="danger" disabled={Boolean(buttonAction)} onClick={() => { if (!window.confirm(`Delete ${quiz.title}? This will move the quiz folder to Trash.`)) return; void runButtonAction("delete-quiz", async () => { const next = await window.getgo.deleteQuiz(quiz.manifestPath); const parentRoute = `/quizzes/contests/${encodeURIComponent(quiz.contest)}`; onRouteChange(parentRoute); setPage({ kind: "contest", contest: quiz.contest }); onSnapshotChange(next); toast.show({ title: "Quiz deleted", description: `${quiz.title} was moved to Trash.` }) }) }}>Delete quiz</Button> : questionOrder ? <><Button variant="outline" color="neutral" disabled={Boolean(buttonAction)} onClick={() => setQuestionOrder(null)}>Cancel</Button><Button icon={<Save size={15} />} variant="solid" loading={buttonAction === "save-question-order"} disabled={Boolean(buttonAction)} onClick={() => void runButtonAction("save-question-order", async () => { const result = await window.getgo.reorderQuizQuestions(quiz.manifestPath, questionOrder); setQuestionRecords(result.questions); setQuestionOrder(null); onSnapshotChange(result.snapshot); toast.show({ title: "Question order saved", description: "Only files inside questions/ were renumbered." }) })}>Save order</Button></> : <><Button icon={<Plus size={15} />} variant="solid" loading={buttonAction === "create-question"} disabled={sourceLoading || Boolean(buttonAction)} onClick={() => void runButtonAction("create-question", async () => { const result = await window.getgo.createQuizQuestion(quiz.manifestPath); const nextRecords = [...questionRecords, result.question]; setQuestionRecords(nextRecords); onSnapshotChange(result.snapshot); const index = nextRecords.length - 1; setSelectedQuestion(index); setQuestionDraftRecord(structuredClone(result.question)); setPendingQuestionNo(String(result.question.question_no)); setQuestionEditorTab("static"); toast.show({ title: `Question ${result.question.question_no} created`, description: "A new file was added to questions/." }) })}>Add question</Button><ActionMenu disabled={sourceLoading || Boolean(buttonAction)} items={[{ id: "reorder", label: "Reorder questions", icon: ListOrdered, disabled: questions.length < 2, onSelect: () => setQuestionOrder(questions.map(question => question.number)) }, { id: "ai-migrate", label: "AI migrate", icon: Zap, onSelect: () => void runButtonAction("ai-migrate", async () => { const job = await window.getgo.startAiMigrationJob({ manifestPath: quiz.manifestPath, context: { contestId: quiz.contest, quizId: quiz.id, title: quiz.title, year: quiz.year, grade: quiz.grade, round: quiz.round } }); toast.show({ title: "AI migration queued", description: `${job.quizTitle} was added to Jobs.` }) }) }]} /></>} />
