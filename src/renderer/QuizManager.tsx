...
import { ContestCrudDialog, QuizCrudDialog } from "./CrudDialogs"
...
export function QuizManager({ snapshot, onChangeRepository, onSnapshotChange }: QuizManagerProps) {
  ...
  return <section className="manager">
    ...
    {contestDialog && <ContestCrudDialog contest={contestDialog === "create" ? undefined : contestDialog} onClose={() => setContestDialog(null)} onSaved={async settings => { const next = contestDialog === "create" ? await window.getgo.createContest(settings) : await window.getgo.updateContest(contestDialog.id, settings); onSnapshotChange(next); setContestDialog(null) }} onDeleted={contestDialog !== "create" ? async () => { const next = await window.getgo.deleteContest(contestDialog.id); onSnapshotChange(next); setContestDialog(null); setPage({ kind: "contests" }) } : undefined} />}
    {quizDialog === "create" && isContest && <QuizCrudDialog onClose={() => setQuizDialog(null)} onSaved={async (input: QuizCrudInput) => { const next = await window.getgo.createQuiz(page.contest, input); onSnapshotChange(next); setQuizDialog(null) }} />}
  </section>
}
