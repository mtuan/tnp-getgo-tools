...
export function QuizManager({ snapshot, initialRoute, onSnapshotChange, onRouteChange, onBackActionChange }: QuizManagerProps) {
  ...
  if (page.kind === "quiz") {
    ...
    return <section className="manager editor-page">
      ...
      <DataTable ariaLabel="Quiz questions" rows={displayedQuestions} columns={questionColumns} rowKey={(item, index) => `${item.number}-${index}`} emptyText={sourceLoading ? "Loading questions…" : "No questions found in questions/ or raw.json."} onRowClick={questionOrder ? undefined : item => setPreviewQuestion(item.record)} onRowMove={questionOrder ? moveOrderedQuestionTo : undefined} /></>}
