...
const sha256 = async (source: string) => Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(source)))).map(byte => byte.toString(16).padStart(2, "0")).join("")
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "generate") { const result = await window.getgo.createDynamicQuestionProposal({ question: record, context, instructions: instructions.trim() || undefined }); if (version !== requestVersion.current) return; await applyGenerated(result, startedAt, version); if (version !== requestVersion.current) return; toast.show({ title: "AI proposal applied", description: result.proposal.warnings[0] ?? result.proposal.explanation }) }
        ...
        const originResult = await builder.generateOriginal(QuizTsService.composeTemplateSource(advancedDynamic))
        if (!originResult) throw new Error("AI fix could not reproduce the original question from its origin parameters.")
        const originalContent = protectedQuestionContent(record)
        const generatedContent = protectedQuestionContent(originResult.question as unknown as Record<string, unknown>)
