...
export function StaticQuestionEditor({ record, manifestPath, onChange }: { record: QuizQuestionRecord; manifestPath: string; onChange(record: QuizQuestionRecord): void }) {
  ...
  const answerType = staticAnswerType(answer.type, Boolean(answer.choices && Object.keys(answer.choices).length))
  const AnswerDetails = answerDetailsComponents[answerType]
  useEffect(() => setPreview(questionService.loadStatic(record)), [record])
  const values = { category: record.category, text_en: record.text_en, text_vn: record.text_vn, explanation_en: explanation.en, explanation_vi: explanation.vi, answer_type: answerType }
  const fields: FormSchema[] = [
    { name: "category", label: "Category", type: "text" },
    { name: "text_en", label: "English question", type: "textarea", rows: 5 },
    { name: "text_vn", label: "Vietnamese question", type: "textarea", rows: 5 },
    { name: "explanation_en", label: "English explanation", type: "textarea", rows: 4 },
    { name: "explanation_vi", label: "Vietnamese explanation", type: "textarea", rows: 4 },
    { name: "answer_type", label: "Answer type", type: "select", options: answerTypeDefinitions.map(definition => ({ value: definition.id, label: definition.label })) },
  ]
  const updateQuestion = (name: string, value: unknown) => {
    if (name === "explanation_en" || name === "explanation_vi") { onChange({ ...record, explanation: { ...explanation, [name === "explanation_en" ? "en" : "vi"]: value } }); return }
    if (name === "answer_type") {
      if (value === answerType) return
      const firstCorrect = Array.isArray(answer.correct) ? answer.correct[0] ?? "" : answer.correct ?? ""
      onChange({ ...record, answer: value === "choice" ? { ...answer, type: "choice", correct: "", choices: answer.choices ?? {} } : { ...answer, type: "input", correct: firstCorrect, choices: undefined, inputType: answer.inputType ?? (answer.type === "numeric" ? "number" : "text") } })
      return
    }
    onChange({ ...record, [name]: value })
  }

  return <div className="advanced-question-layout static-question-layout">
