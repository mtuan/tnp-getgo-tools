...
export function Select({ value, options, onValueChange, placeholder = "Select…", disabled = false, autoFocus = false, color = "normal", className, title }: { value?: string; options: SelectOption[]; onValueChange(value: string): void; placeholder?: string; disabled?: boolean; autoFocus?: boolean; color?: SelectColor; className?: string; title?: string }) {
  ...
  return <div className={["schema-select", `select-color-${color}`, dropdown.open && "open", dropdown.openUp && "open-up", className].filter(Boolean).join(" ")} ref={dropdown.ref}>
    <button type="button" title={title} disabled={disabled} autoFocus={autoFocus} aria-haspopup="listbox" aria-expanded={dropdown.open} onClick={() => dropdown.setOpen(current => !current)}><span className={!selected ? "placeholder" : ""}>{selected?.label ?? placeholder}</span><ChevronDown /></button>
    {dropdown.open && createPortal(<div ref={dropdown.menuRef} className={`schema-select-menu schema-select-portal select-menu-color-${color}`} style={{ left: dropdown.position.left, width: dropdown.position.width, top: dropdown.openUp ? "auto" : dropdown.position.top, bottom: dropdown.openUp ? dropdown.position.bottom : "auto" }} role="listbox">{options.map(option => <button type="button" role="option" aria-selected={option.value === value} className={option.value === value ? "selected" : ""} onClick={() => { onValueChange(option.value); dropdown.setOpen(false) }} key={option.value}><span>{option.label}</span>{option.value === value && <Check />}</button>)}</div>, document.body)}
  </div>
}
