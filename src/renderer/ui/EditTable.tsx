...
export function EditTable<T extends object>({
  columns,
  rows,
  rowKey,
  onRowChange,
  ...
  useLayoutEffect(() => {
    if (focusAddedRow.current && rows.length > previousRowCount.current) {
      const selector =
        "tbody tr:last-child td:not(.edit-table-reorder):not(.edit-table-actions) input:not(:disabled), tbody tr:last-child td:not(.edit-table-reorder):not(.edit-table-actions) textarea:not(:disabled), tbody tr:last-child td:not(.edit-table-reorder):not(.edit-table-actions) button:not(:disabled)";
  ...
  return (
    <div className="edit-table" ref={rootRef}>
      <div className="edit-table-scroll">
        <table aria-label={ariaLabel}>
          <thead>
            <tr>
              {reorderable && (
                <th className="edit-table-reorder">
                  <span className="sr-only">Reorder</span>
                </th>
              )}
              {columns.map((column) => (
                <th style={{ width: column.width }} key={column.key}>
                  {column.title}
                </th>
              ))}
              {onRowDelete && (
                <th className="edit-table-actions">
            ...
            {rows.map((row, rowIndex) => {
              ...
              return (
                <tr
                  ...
                  onDrop={(event) => {
                    event.preventDefault();
                    if (dragging !== null) move(dragging, rowIndex);
                    setDragging(null);
                    setDragOver(null);
                  }}
                ...
                >
                  {reorderable && (
                    <td className="edit-table-reorder">
                      <Button
                        variant="icon"
                        draggable
                        aria-label={`Reorder row ${rowIndex + 1}. Use arrow keys or drag.`}
                        icon={<GripVertical />}
                        onDragStart={(event) => {
                      ...
                      {column.renderView ? (
                        ...
                        <FormControl
                          ...
                          onChange={(_name, value) =>
                            onRowChange(rowIndex, column.dataKey, value)
                          }
                        />
                      )}
                    </td>
      ...
      {onRowAdd && (
        <Button
          variant="text"
          className="edit-table-add"
          icon={<Plus />}
          onClick={() => {
            focusAddedRow.current = autoFocusOnAdd;
            onRowAdd();
          }}
        >
          {addLabel}
