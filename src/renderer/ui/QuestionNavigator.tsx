...
export function QuestionNavigator({ value, items, disabled = false, onValueChange }: {
  ...
  const optionLabel = (item: QuestionNavigationItem) => <span className="ui-question-navigator-option"><strong>{item.label}</strong></span>
  return <div className={`ui-question-navigator ${dropdown.open ? "open" : ""}`} aria-label="Question navigation" ref={dropdown.ref}>
    ...
    <button className="ui-question-navigator-trigger" type="button" disabled={disabled} aria-label="Select question" aria-haspopup="listbox" aria-expanded={dropdown.open} onClick={() => dropdown.setOpen(open => !open)}><strong>{selected?.label ?? "Select question"}</strong></button>
