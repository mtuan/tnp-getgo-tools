...
const parseNumbers = (value: string) => parseStrings(value).map(Number).filter(value => Number.isInteger(value))
...
export function ContestSettingsDialog({ contest, onClose, onSaved, onDeleted }: { contest?: ContestSummary; onClose(): void; onSaved(settings: ContestSettings): Promise<void>; onDeleted?: () => Promise<void> }) {
  ...
  return <DialogFrame presentation={contest ? "drawer" : "modal"} title={contest ? `Edit ${contest.title}` : "Create contest"} busy={busy} error={error} onClose={onClose} onSubmit={submit} onDelete={onDeleted ? async () => { setBusy(true); try { await onDeleted() } catch (cause) { setBusy(false); setError(cause instanceof Error ? cause.message : String(cause)) } } : undefined}>
    ...
    <div className="settings-tab-content" key={tab}>
      ...
      {tab === "grades" && <><div className="section-heading"><div><h3>Grade mappings</h3><p>Display levels mapped to numeric school grades.</p></div><button type="button" className="secondary" onClick={() => add("grades", { gradeName: "", grades: [] })}><Plus />Add grade</button></div>{!grades.length ? <EmptySection label="grades" onAdd={() => add("grades", { gradeName: "", grades: [] })} /> : <div className="settings-list">{grades.map((grade, index) => <article className="settings-item compact" key={index}><ItemHeader title={text(grade.gradeName) || `Grade ${index + 1}`} detail={strings(grade.grades)} onDelete={() => remove("grades", index)} /><div className="field-grid"><label>Display name<input required placeholder="Benjamin" value={text(grade.gradeName)} onChange={event => update("grades", index, { gradeName: event.target.value })} /></label><label>Numeric grades <span className="label-help">Comma separated</span><input required placeholder="5, 6" value={strings(grade.grades)} onChange={event => update("grades", index, { grades: parseNumbers(event.target.value) })} /></label></div></article>)}</div>}</>}
