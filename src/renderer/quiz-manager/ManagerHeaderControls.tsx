...
import type { BackgroundJob, RepositorySnapshot } from "../../core/models";
...
function useMarketplaceSync(locale: Context["locale"], toast: Context["toast"]) {
  ...
  const start = async () => {
    try { setJob((await window.getgo.syncContentV2Marketplace()).jobs.find(isSyncJob) ?? null); }
...
export function ManagerHeaderControls(context: Context) {
  ...
  const items = [
    ...
    ...(!isContest ? [{ id: "sync", label: sync.label, icon: RefreshCw, disabled: Boolean(sync.job), onSelect: () => void sync.start() }, { id: "view", label: topicsView === "tree" ? "Show list view" : "Show tree view", icon: topicsView === "tree" ? Rows3 : ListOrdered, onSelect: () => { const next = topicsView === "tree" ? "list" : "tree"; setTopicsView(next); try { localStorage.setItem("getgo-tools.topics-view", next); } catch { /* optional */ } } }] : []),
  ];
  return <>
