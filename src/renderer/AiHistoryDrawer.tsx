...
type StoredGenerate = DynamicQuestionProposalResult & { generatedAt: string; processingTimeMs?: number }
type StoredFix = DynamicQuestionFixResult & { generatedAt: string; processingTimeMs?: number; proposal?: DynamicQuestionProposal }
...
export function AiHistoryDrawer({ record, onClose }: { record: QuizQuestionRecord; onClose(): void }) {
  ...
  return <DialogFrame presentation="drawer" className="ai-history-drawer" hideFooter title="AI generation history" busy={false} error={null} onClose={onClose} onSubmit={event => event.preventDefault()}>
    ...
    <div className="ai-history-content">
      {tab === "summary" && <div className="ai-response-summary-panel"><section><strong>Model</strong><p>{revision.model}</p></section><section><strong>Confidence</strong><p>{Math.round(revision.proposal.confidence * 1000) / 10}%</p></section><section><strong>Processing time</strong><p>{processingTime(revision.processingTimeMs)}</p></section><section><strong>Total tokens</strong><p>{revision.usage.totalTokens.toLocaleString()}</p></section><section><strong>Input tokens</strong><p>{revision.usage.inputTokens.toLocaleString()}</p></section><section><strong>Output tokens</strong><p>{revision.usage.outputTokens.toLocaleString()}</p></section><section><strong>Cached input</strong><p>{revision.usage.cachedInputTokens.toLocaleString()}</p></section><section><strong>Cache write</strong><p>{revision.usage.cacheWriteTokens.toLocaleString()}</p></section><section className="wide"><strong>Explanation</strong><p>{revision.proposal.explanation}</p></section>{revision.proposal.warnings.length > 0 && <section className="wide warning"><strong>Warnings</strong><ul>{revision.proposal.warnings.map((value, index) => <li key={`${value}-${index}`}>{value}</li>)}</ul></section>}{revision.proposal.assumptions.length > 0 && <section className="wide"><strong>Assumptions</strong><ul>{revision.proposal.assumptions.map((value, index) => <li key={`${value}-${index}`}>{value}</li>)}</ul></section>}</div>}
