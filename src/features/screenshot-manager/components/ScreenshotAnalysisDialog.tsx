...
export function ScreenshotAnalysisDialog({ locale, documents, onClose }: { locale: "en" | "vi"; documents: ScreenshotAnalysisDocuments; onClose(): void }) {
  const vi = locale === "vi";
  const [view, setView] = useState<"rules" | "page">("rules");
  ...
  return <ui.DialogFrame className="screenshot-analysis-dialog" presentation="drawer" title={vi ? "Phân tích giao diện" : "UI analysis"} busy={false} error={null} hideFooter onClose={onClose} onSubmit={preventSubmit}>
    <ui.Tabs value={view} onChange={value => setView(value as "rules" | "page")} ariaLabel={vi ? "Tài liệu phân tích" : "Analysis documents"} items={[{ id: "rules", label: vi ? "Quy tắc chung" : "General rules" }, { id: "page", label: vi ? "Bố cục trang" : "Page layouts" }]} />
    {view === "rules" ? <pre className="screenshot-analysis-document screenshot-analysis-markdown">{documents.generalRulesMarkdown}</pre> : <div className="screenshot-analysis-page-viewer">
