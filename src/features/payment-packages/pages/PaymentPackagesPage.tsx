...
import type { AppSettings, PaymentPackage } from "../../../shared/domain/models";
...
export function PaymentPackagesPage({ locale }: { locale: AppSettings["locale"] }) {
  ...
  const text = isVi ? { title: "Gói thanh toán", description: "Quản lý gói thành viên và đồng bộ với Firestore.", add: "Thêm gói", sync: "Đồng bộ", name: "Tên", type: "Loại", info: "Thông tin", benefits: "Quyền lợi", price: "Giá", save: "Lưu", create: "Tạo", cancel: "Hủy" } : { title: "Payment packages", description: "Manage membership packages and synchronize them with Firestore.", add: "Add package", sync: "Sync", name: "Name", type: "Type", info: "Information", benefits: "Benefits", price: "Price", save: "Save", create: "Create", cancel: "Cancel" };
  ...
  const fields = useMemo<ui.FormSchema[]>(() => [
    ...
    { type: "select", name: "type", label: text.type, options: [{ value: "monthly", label: isVi ? "Hàng tháng" : "Monthly" }, { value: "annual", label: isVi ? "Hàng năm" : "Annual" }, { value: "one-time", label: isVi ? "Một lần" : "One-time" }] },
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); if (!draft || !items) return; setBusy(true); try { const next = [...items.filter((item) => item.id !== originalId), draft].sort((a, b) => a.name[locale].localeCompare(b.name[locale])); setItems(await window.getgo.savePaymentPackages(next)); setDraft(null); toast.show({ title: isVi ? "Đã lưu gói" : "Package saved", variant: "success" }); } catch (error) { toast.show({ title: isVi ? "Không thể lưu" : "Save failed", description: String(error), variant: "error" }); } finally { setBusy(false); } };
  ...
  const remove = async () => { if (!originalId || !items) return; setBusy(true); try { setItems(await window.getgo.savePaymentPackages(items.filter((item) => item.id !== originalId))); setDraft(null); toast.show({ title: isVi ? "Đã xóa gói" : "Package deleted", variant: "success" }); } finally { setBusy(false); } };
  ...
  return <section><ui.PageHeader eyebrow="Billing" title={text.title} description={text.description} actions={<ui.ControlGroup><ui.Button icon={<Plus />} onClick={() => { setOriginalId(null); setDraft(emptyPackage()); }}>{text.add}</ui.Button><ui.Button icon={<RefreshCw />} loading={busy} onClick={sync}>{text.sync}</ui.Button></ui.ControlGroup>} />
    {items === null ? <div className="ui-panel-loading"><span className="mini-spinner" /></div> : <ui.DataTable rows={items} columns={columns} rowKey={(item) => item.id} ariaLabel={text.title} emptyText={isVi ? "Chưa có gói thanh toán." : "No payment packages."} />}
