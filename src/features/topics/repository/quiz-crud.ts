...
export async function createContestDirectory(
  root: string,
  requestedSettings: ContestSettings,
): Promise<void> {
...
export async function updateContestSettings(
  ...
  if (settings.book.code !== id)
    throw new Error("Contest code cannot be changed after creation.");
...
export async function renameContestDirectory(
  root: string,
  currentId: string,
  requestedId: string,
): Promise<void> {
  ...
  if (!(await pathExists(currentDirectory)))
    throw new Error(`Contest “${current}” does not exist.`);
  if (await pathExists(nextDirectory))
    throw new Error(`Contest “${next}” already exists.`);
  ...
  await fs.writeFile(
    settingsPath,
    JSON.stringify(
      { ...settings, book: { ...settings.book, code: next } },
      null,
      2,
    ) + "\n",
    "utf8",
  ...
  for (const entry of entries) {
    ...
    const manifest = quizManifestSchema.parse(
      JSON.parse(await fs.readFile(manifestPath, "utf8")),
    );
    await fs.writeFile(
      manifestPath,
      JSON.stringify({ ...manifest, contest: next }, null, 2) + "\n",
      "utf8",
    );
  }
}

export async function createQuizFiles(
  root: string,
  contestId: string,
  input: QuizCrudInput,
): Promise<void> {
  ...
  if (!(await pathExists(contestDirectory)))
    throw new Error(`Contest “${contest}” does not exist.`);
      ...
      ...(manifest.type === "alphabet"
        ? [
            fs.writeFile(
              ...
              JSON.stringify({ schemaVersion: 1, words: [] }, null, 2) + "\n",
              "utf8",
            ),
          ]
        : []),
...
export async function updateQuizManifest(
  manifestPath: string,
  ...
  const updated: QuizManifest = {
    ...
    quizBuilderApiVersion:
      input.quizBuilderApiVersion ?? manifest.quizBuilderApiVersion,
  };
  updated.source.quizTsSha256 = createHash("sha256")
    .update(quizSource)
    .digest("hex");
  await Promise.all([
...
export async function updateQuizSource(
  manifestPath: string,
  source: string,
): Promise<void> {
  const manifest = quizManifestSchema.parse(
  ...
  manifest.source.quizTsSha256 = createHash("sha256")
    .update(source)
    .digest("hex");
  await Promise.all([
    fs.writeFile(
      path.join(path.dirname(manifestPath), "quiz.ts"),
      source,
      "utf8",
    ),
    fs.writeFile(
